...
>
> **实现说明（v0.8.1）：** PRD 中描述的"飞书审批"已由 Web Dashboard 替代为主要操作界面。
> 超时自动确认（30 分钟）、超时自动跳过（2 小时）已实现（`background.py`）。
> SSRF 防护已加固（`routes/reader.py`）。质量门阈值已更新为三级配置。
...
| **后端 API** | FastAPI (Python 3.14+) | Dashboard 后端 + 知识库服务 |
...
**超时策略：** 30分钟未确认 → 自动选最高分选题（已实现，`background.py` `topic_timeout_target`）
...
| ③ AI腔审校 | 正则（23 规则）+ LLM双检测去AI味 | 审校后文稿 | 审校分数<60→回退 |
...
上午篇：
  ...
  09:30  人工确认选题（超时30分→自动选最高分，已实现）
  09:30-10:30  Writer 写作（7阶段管线，3个标准版本）
  10:45  Dashboard 审批通知
  10:45-11:00  人工审批（超时2小时→跳过，已实现）
  11:00  出稿完成
...
| 场景 | 策略 |
|------|------|
| 选题超时未确认（>30分） | 自动选评分最高选题（已实现，`background.py`） |
| 审批超时无响应（>2小时） | 跳过该时段（已实现，`background.py`） |
...
配置文件：`config/quality_gates.json`

| 阈值 | 配置键 | 默认值 | 可调范围 |
|------|--------|--------|---------|
| AI腔审校及格线 | `proofread_threshold` | 60分 | 40-90 |
...
- **人工抽检机制（#6）**：每周随机抽取 3 篇已发布文章，人工评分后与 LLM 评分对比，偏差 > 15% 时调整审校 prompt（✅ 已实现：`dashboard/backend/routes/reviews.py` + `database/manual_reviews.py`）
