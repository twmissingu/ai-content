...
| 信源流 | 实时刷新、评分+来源+新鲜度、搜索过滤 | `/sources` 页面：筛选表格、聚合统计 | 已实现 |
| 阅读+编辑 | 原文和作品并排对照 | ApprovalView 内联编辑 + ReaderPanel 原文阅读 | 已实现 |
| 面板控制 | 可折叠、全屏、并排 | 三栏布局切换 + ResizablePanel | 已实现 |
...
| 饱和度 | 实体聚类 + Jaccard>25% | 实体聚类 + Jaccard>25% | 已实现 |
...
| 维度 | 文章做法 | ai-content 现状 | 差距 |
|------|---------|----------------|------|
| 原文阅读 | 阅读器 + 高亮 + 笔记 + 翻译 | ReaderPanel 原文代理抓取+面板阅读 | 已实现（简化版） |
| 协作模式 | 读→标注→AI 写→对照改 | AI 写→审批→编辑→发布 | 已改进 |
| 审批界面 | 阅读器+编辑器并排 | 内联 Markdown 编辑器 + 实时预览 | 已实现 |
...
| 自动沉淀 | 高亮/笔记自动入 KB | LLM 分析提取关键词/标签/写作模式 | 已实现 |
| KB 浏览 | 可浏览+搜索 | 目录树 + 文件查看器 + 搜索 | 已实现 |
| 爆款分析 | 自动提取标题模式和关键词 | knowledge.py AI 分析（keywords/tags/patterns） | 已实现 |
...
|------|---------|----------------|------|
| 平台数 | 头条为主 | 6 平台 | **项目更全面** |
| 异步 Job | 后台子进程 + SSE 进度 | 同步子进程（90s timeout）+ per-platform 状态 | 已改进 |
...
- **方案**：textarea + marked 实时预览 + DOMPurify，保存时调用 PUT endpoint
- **改动量**：前端 ~200 行 + 后端 ~50 行
- **预计工时**：4-6h
- **状态**：已实现 — `GET/PUT /api/approval/article/{id}/content` + 内联编辑器
...
- **方案**：后端新增 `/api/kb/tree` + `/api/kb/file`，前端增加侧边栏树形导航
- **改动量**：前端 ~150 行 + 后端 ~80 行
- **预计工时**：3-4h
- **状态**：已实现 — 3 层目录树 + 文件查看器 + 路径遍历防护
...
- **方案**：Scout 保存原始抓取结果到 `queue/sources/`，前端新增 `/sources` 筛选表格
- **改动量**：前端 ~300 行 + 后端 ~100 行 + scout.py ~30 行
- **预计工时**：6-8h
- **状态**：已实现 — `SOURCES_DIR` + `_save_raw_sources()` + `GET /api/sources` + SourcesView
...
- **改动量**：~80 行
- **预计工时**：3-4h
- **状态**：已实现 — `_analyze_article()` + `_save_meta()` + LLM 分析失败不阻塞归档
...
- **范围**：前端整体布局重构
- **现状**：单栏卡片堆叠，6 个独立页面
- **目标**：可选的三栏布局（概览 / 主内容区 / 快捷操作），面板可折叠
- **方案**：ResizablePanel 组件 + App.vue 布局切换按钮
- **预计工时**：2-3 天
- **状态**：已实现（简化版）— 左侧概览面板 + 右侧快捷导航 + 切换按钮

#### 11. 原文阅读 + 标注系统 ✅（简化版）

- **范围**：新增阅读器组件 + 后端代理
- **现状**：无原文阅读视图，无标注能力
- **目标**：选中信源后展示原文（简化版不含标注系统）
- **方案**：`GET /api/reader/fetch` 代理抓取 + ReaderPanel 滑出面板
- **预计工时**：1 周
- **状态**：已实现（简化版）— TopicsView 集成阅读按钮，ReaderPanel 滑出面板

#### 12. 发布状态细化 ✅

- **范围**：publisher 进度报告
- **现状**：同步子进程（60s timeout），无逐平台进度
- **目标**：每个平台发布前后报告状态，timeout 增加到 90s
- **方案**：发布前后调用 `self.write_status()` + timeout 60→90
- **预计工时**：2-3 天
- **状态**：已实现 — per-platform 状态报告 + timeout 90s
...
- **预计工时**：1-2 天
- **状态**：已实现 — `extract_keywords()` 实体识别 + 阈值 0.3→0.25
