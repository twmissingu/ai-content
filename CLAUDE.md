...
- `skills/common.py` — Shared utilities (AgentBase, metrics, status writing) across all agents

## Dashboard Backend Modules

- `dashboard/backend/main.py` — FastAPI app with 6 route groups (`/api/pipeline`, `/api/approval`, `/api/topics`, `/api/data`, `/api/kb`, `/api/config`)
- `dashboard/backend/database.py` — SQLite operations (pipeline sessions, approvals, token usage, config)
- `dashboard/backend/search.py` — Full-text search for knowledge base
